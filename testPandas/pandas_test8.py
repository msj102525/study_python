...
# 데이터프레임 인덱스 조작
# 인덱스 라벨이 없는 데이터프레임에 라벨을 설정하거나 제거하는 것
# 행 인덱스와 열 인덱스를 서로 바꿔야 할 때도 인덱스 조작할 수 있음 (인덱스 교환)
# set_index() 함수 : 기존의 행 인덱스를 제거하고, 데이터 열 중에서 하나를 행 인덱스로 설정할 때 사용함
#                   한 컬럼의 값들 => 행 인덱스 라벨이 됨
# reset_index() 함수 : 기존의 행 인덱스를 제거하고, 인덱스를 데이터 열로 추가함
#                       행 인덱스 라벨 => 한 컬름의 값들로 바뀜
...
# set_index() 함수
# 기존의 행 인덱스는 지우고, 특정 열 데이터를 행인덱스 라벨로 지정잫ㄹ 때 사용하는 함수임
df2 = df1.set_index('C1')   # 'C1' 컬럼을 행인덱스 라벨로 지정
print(df2)

print(df2.set_index('C2'))

# reset_index() 함수
# 기존의 행인덱스 라벨을 컬럼 값으로 바꿈
...
# 데이터프레임 생성할 때 column 인수에 리스트의 리스트(행렬)로 인덱스 설정한 경우
np.random.seed(0)
df3 = pd.DataFrame(np.round(np.random.randn(5, 4), 2), columns=([['A', 'A', 'B', 'B'], ['C1', 'C2', 'C3', 'C4']]))
print(df3)

# columns_names 속성
# 다중 인덱스에 이름 지정시 사용함, 이름들은 리스트로 설정함
