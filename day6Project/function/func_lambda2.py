...
def test_sorted():
    student = [('김영희', 'A', 95), ('홍철수', 'B', 96), ('이길동', 'C', 90)]

    print(student)
    print(sorted(student))  # 키 지정 누락시 자동 각 아이템의 [0]번째가 키로 지정됨
    print(sorted(student, key=lambda x: x[1]))  # 각 아이템의 [1]번째가 정렬의 키가 됨
    print(sorted(student, key=lambda x: x[2]))  # 각 아이템의 [2]번째가 정렬의 키가 됨

    # sorted() 키 지정시
    # 파이썬이 제공하는 operator 모듈을 이용할 수도 있음
    print(sorted(student, key=operator.itemgetter(2)))  # [2]번째가 정렬의 키가 됨

    # 내림차순정렬 : reverse=True 로 지정함
    # 점수 기준 내림차순정렬을 한다면
    print(sorted(student, key=operator.itemgetter(2), reverse=True))  # [2]번째가 정렬의 키가 됨
# ----------------------------------------
...
def list_lambda():
    ...
    print([(lambda d, s: f'{d} * {s} = {d * s}')(dan, su) for dan in range(2, 10) for su in range(1, 10)])   # 람다 함수
