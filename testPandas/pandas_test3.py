...
# 데이터들을 가지고 표(테이블)을 만들 때 사용하는 클래스임
'''
가장 간단한 데이터프레임 생성 방법
1. 딕셔너리 만든다. => 키: 값 (리스트나 일차원배열로 작성함 => 컬럼(열)이 됨)
    키(key) => 커럼 라벨이 됨 (기본 문자열임)
...
# 시리즈처럼 인덱싱 사용 : 컬럼라벨, 인덱스라벨을 사용할 수 있음
# 예 : "2010-2015 증가율" 이라는 이름의 열값을 변경한다면
df["2010-2015 증가율"] = df["2010-2015 증가율"] * 100  # 라벨만 있으면 변경임
print(df)
df["2019-2022 증가율"] = ((df["2022"] - df["2019"]) / df["2019"] * 100).round(2)
print(df)  # 없는 라벨에 대해 대입하면 추가됨

# 데이터플임의 열삭제, del 명령 사용함
del df["2019-2022 증가율"]
print(df)

# 데이터프레임에서는 하나의 키가 하나의 열(컬럼)이 되므로, 열단위로 인덱싱 가능함
# 컬럼라벨(키)을 이용하면 됨
print(df['지역'])

# 여러 개의 컬럼(열)을 인덱싱 할 수도 있음
...
# 데이터프레임에서 일부를 추출해서 별도의 시리즈를 생성
s = df['2019']  # [] 한겹 사용
print(s)
print(type(s))

# 인덱싱시 주의사항
# 정수 인덱스는 데이터프레임에서는 행 인덱스에만 사용할 수 있음
# 열(column) 인덱스에 정수 인덱스 사용하면 에러남 (KeyError 발생함)
# print(df[0]) # 0번쨰 컬럼 출력 => KeyError

# 데이터프레임 컬럼에 정수 인덱스가 가능한 경우는, 숫자형 라벨을 지정한 경우에는 가능함
df3 = pd.DataFrame(np.arange(12).reshape(3, 4))
print(df3)
print(df3[2])
print(df3[[1, 2]])  # ar[1][2] == ar[1, 2] : 1행2열, ar[[1, 2]] : 1열과 2열 선택

# 행 인덱싱 : 항상 슬라이싱해야 함 (라벨로 슬라이싱할 수도 있음)
print(df[:1])  # 0행 추출
print(df[1:2])  # 1행 추출
