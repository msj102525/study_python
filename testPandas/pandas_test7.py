...
# 타임스템프 인덱스 라벨은 일정한 간격을 유지할 필요는 없음
# DatetimeIndex 생성함수 : pd.to_datetime(), pd.date_range() 함수 사용함

# pd.to_datetime() 함수
# 날짜, 시간을 나타내는 문자열을 자동으로 datetime 자료형으로 바꾼 다음,
# DatetimeIndex 를 생성하는 함수임
date_str = ['2018. 1. 4', '2018. 1. 5', '2018. 1. 6']
idx = pd.to_datetime(date_str)
print(idx)  # DatetimeIndex(['2018-01-04', '2018-01-05', '2018-01-06'], dtype='datetime64[ns]', freq=None)
...
# WOM-2THU : 각 달의 두번쨰 목요일
# QE-JAN : 각 분기의 첫달의 마지막 날
# QE-DES : 각 분기의 마지막 달의 마지막 말
...
print(pd.date_range('2020-4-1', '2020-4-30', freq='BMS'))
print(pd.date_range('2019-4-1', '2020-4-30', freq='WOM-2THU'))
print(pd.date_range('2020-4-1', '2020-4-30', freq='QE-JAN'))
...
print(ts.shift(1, freq='W'))    # 각 달의 첫주의 일요일 날짜로 이동됨(변경됨)
...
# 예, 10분간격으로 구간을 만들면 10의 배수가 되는 시각은 다음 구간의 시작점이 됨
...
print(ts.resample('10min', closed='right').sum())   # 0분 ~ 9분까지 합계
...
# 뒤 데이터를 앞에서 미리 쓰는 backword filling 방식이 있음
# bfill(), ffill() 제공됨
print(ts.resample('30s').ffill().head(20))
print(ts.resample('30s').bfill().head(20))

# dt 접근자
# datetime 자료형 시리즈에 dt 접근자 제공됨, 속성과 메서드도 제공됨
...
# strftime() 함수 : 포맷(format)을 이용해서 문자열 형태로 변환하는 함수
