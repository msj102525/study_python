...
# reshape() 사용시에 면, 행, 열의 갯수를 지정하지 않고, -1로 표기할 수도 있음
# -1로 표시된 항목은 계산에 의해 갯수가 설정됨
bar2 = ar.reshape(3, -1)
print(bar2)
...
print(bar.flatten())    # 2차원배열 => 1차원배열
print(bar.ravel())

print('bar3 : ', bar3.shape)    # bar : (3, 4)
print(bar3.flatten())   # 3차원배열 => 1차원배열
print(bar3.ravel())

# newaxis() 함수
# 배열의 차원을 1증가 시키는 함수
# 1차원배열 => 2차원배열로, 2차원배열 => 3차원배열로
# 값의 갯수가 5개인 1차원배열의 경우, 2차원배열로 바꿀 때 (5, 1) 과 (1, 5)로 변경 가능함
