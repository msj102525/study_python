...
# Series : numpy 의 1차원배열 또는 리스트 + 인덱스라벨 지정한 것
...
# 자동으로 0으로 시작하는 정수가 인덱스라벨러 사용됨
print(pd.Series(range(10, 14)))

# 2. DataFrame (데이터프레임 : 표 - 행렬 : 2차원배열, 매트릭스) 만들기
# 변수 = pd.DataFrame(행렬변수, index=행인덱스라벨, columns=열컬럼라벨)
data = {
...
# 출력시 컬럼 나열 순서를 재배치할 때 DataFrame 사용할 수 있음 => 표 형태를 만들고자 한다면
# 컬럼라벨
...
# columns 생략되면, 위의 데이터 구성 순서대로 프레임이 구성됨
index = ['서울', '부산', '인천', '대구']
df2 = pd.DataFrame(data, index=index)
print(df2)  # key 가 컬럼라벨이 됨

df3 = pd.DataFrame(np.arange(12).reshape(3, 4))
print(df3)

# 파일 입출력 기능 제공
# 파일에 기럭(저장) : 데이터프레임변수.to_파일종류('파일명.확장자' , 속성='값', .....)
# df.to_csv('sample.cxv', mode='w', sep=',')

# 파일로 부터 읽어와서 DataFrame 에 저장
# 데이터프레임변수 = pd.read_파일종류('파일명.확장자', 속성='값', ....)
df4 = pd.read_csv('sample.cxv')
print(df4)
