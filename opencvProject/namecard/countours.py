...
# 이진화 처리
th, src_bin = cv2.threshold(src_grey, 0, 255, cv2.THRESH_BINARY | cv2.THRESH_OTSU)

# 외곽선 검출 : 모드를 다르게 해서 2가지 출력 확인
countours1, _ = cv2.findContours(src_bin, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_NONE)
countours2, _ = cv2.findContours(src_bin, cv2.RETR_LIST, cv2.CHAIN_APPROX_NONE)

# 외곽선 그리기
'''
for i in range(len(countours1)):
    random_color = (random.randint(0, 255), random.randint(0, 255), random.randint(0, 255))
    cv2.drawContours(dst1, countours1, i, random_color, 1)  # 테두리선 그리기

for i in range(len(countours2)):
    random_color = (random.randint(0, 255), random.randint(0, 255), random.randint(0, 255))
    cv2.drawContours(dst2, countours2, i, random_color, 1)  # 테두리선 그리기
'''
...
for i in range(len(countours1)):
    ...
    random_color = (random.randint(0, 255), random.randint(0, 255), random.randint(0, 255))
    cv2.drawContours(dst1, countours1, i, random_color, 1)

    # 면적이 너무 작은 객체는 제외시킴
    if cv2.contourArea(pts) < 1000:
        continue    # 아래 내용 실행하지 말고 다음 인덱스의 외곽선으로 넘어가라. 다시 for 문
