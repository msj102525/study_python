...
# [:2] == [0:2] == 0, 1 추출 => 인덱스 0의 값 246이 h 에 대입, 인덱스 1 의 값 300은 w 에 대입
print('index : ', h, ', ', w)
...
for i in range(1, cnt):
    ...
    # 레이블링을 위해 준비된 행렬에 추출된 코인 (흰색영역) 부분에 색을 칠함
...
# print(len(countours))   # 외곽선에 해당되는 픽셀의 위차값들 [[(y행, x열), .....], [...], ......]

# 외곽선에 해당하는 픽셀에 색을 지정함 : dst2 에 외곽선 색 저장
for i in range(len(countours)): # 각 외각선 픽셀에 색칠함
    # 랜덤하게 rgb 색 만듦
    random_color = (random.randint(0, 255), random.randint(0, 255), random.randint(0, 255))
