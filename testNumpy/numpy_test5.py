...
x3 = np.array([1, 2, 3.0])  # 같은 종류가 되기 위해 정수가 실수로 자동 형변환 됨
print(x3.dtype) # float64
print(type(x3))
print(x3)

# 배열 생성시 dtype 인수에 자료형을 지정할 경우
# 제공되는 접두사 뒤에 바이트(또는 비트)수를 지정함 ex) int43, float64
# 문자타입 (char type) 접두사 뒤에는 글자수를 지정할 수 있음
# b : boolean
# i : integer
# f : float
# u : unsigned int (부호없는 정수 : 음수를 양수로 변환한 정수, ex > -128 ~ 127 => unsigned 하면 0 ~ 255)
# 0 ~ 127 그대로 두고, -128 ~ -1 을 127 뒤에 붙임, 0 ~ 127, -128 ~ -1 => -128은 128이 됨, -1이 255가 됨
# c : 복소수 (complex)
# O : Object
...
# 적용 예 : 1 을 0으로 나누려고 하거나, 0에 대한 로그값 계산하면 무한대인 np.inf 가 결과로 표시됨
