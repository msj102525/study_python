...
# 시리즈 연산
# numpy 의 배열 백터화연산을 Series 에도 사용할 수 있음
# 단, 시리즈의 값에만 연산이 허용됨

# 예 : 인구 숫자를 백만단위로 만들기 위해, 시리즈 객체에 1000000 나누기함
print(s / 1000000)

# 시리즈 인덱싱 : numpy 의 배열처럼 인덱싱 가능함, 인덱싱에 인덱스라벨을 사용할 수 있음
# 슬라이싱 시에도 인덱스라벨을 사용할 수 있음
# 인덱싱하면 값이 반환됨
# print(s[1], s['부산'])    # FutureWarning 발생 => 사용이 변경됨
print(s.iloc[3], s.loc['대구'], s.대구) # 사용권장
...
# 슬라이싱
print(s[1:3])   # 인덱스1부터 인덱스3까ㅣ (3은 미포함)
print(s['부산':'대구']) # 부산부터 대구까지 (대구 포함)

# 시리즈와 딕셔너리 자료형
# 시리즈의 인덱스 라벨이 딕셔너리의 키(key)에 해당됨
# 딕셔너리에서 in 과 item() 함수를 시리즈에도 사용할 수 있음 => 시리즈의 인덱스라벨(키)과 값(value)에 접근함
print('서울' in s) # 인덱스라벨 중에 '서울'이 있느냐 : True
print('대전' in s) # False
...
dict = {'서울': 4858294, '부산': 4891931, '인천': 8285011, '대전': 1395039}
print(dict)
s2 = pd.Series(dict)
print(s2)
s2.name = '2020년 인구'
...
print(s4)   # 두시리즈에 라벨이 일치하지 않는 값은 NaN 처리됨

print(s.values - s2.values) # s[0] - s2[0], s[1] - s2[1], .....

# Series.notnull
# 대구와 대전은 두 시리즈에 모두 존재하지 않기 때문에 계산이 불가능하므로 NaN (Not A Number) 이라는 결과가 나옴
# NaN 값은 float 자료형에서만 표현 가능함
# 계산 결과에서 NaN 이 아닌 값들만 구하려면 notnull() 메서드 사용
print(s4.notnull()) # True, False 로 표현됨
print(s4[s4.notnull()])
...
# 시리즈의 데이터 갱신(update), 추가(add), 삭제(delete) 가능함
# 인덱싱을 이용하면 딕셔너리처럼 데이터를 추가, 갱신할 수 있음
s5['부산'] = 1.63 # 인덱스 라벨이 존재하면 갱신(update)
print(s5)

s5['대구'] = 1.45 # 인덱스 라벨이 없으면 추가(add)
print(s5)
