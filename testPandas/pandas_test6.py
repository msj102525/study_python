...
# 데이터프레임에 count 는 각 열의 데이터 갯수를 셈
# 데이터프레임에 값이 누락된 부분을 찾을 때 유용하게 이용함
np.random.seed(2)   # 발생 난수값 고정 알고리즘 지정함
...
# 값을 종류별로 구분한 것 : 카테고리
np.random.seed(1)
s2 = pd.Series(np.random.randint(6, size=100)) # 0~5 사이의 정수 100개 발생
...
# 첫번쨰 열 기준으로 정렬하고 나서, 첫번째 기준열의 값은 값들에 대해 두번쨰 열기준으로 다시 정렬되게 하려면
# by 인수에 리스트를 사용해서 순서대로 열을 나열하면 됨
# print(df.sort_values(by=[1, 2]))

# 행, 열 합계 : sum() 함수
# 행과 열의 합계를 구할 때 sum(axis=숫자) 사용
# asis 인수에는 합계로 없어지는 방향축을 지정함 : 0-행, 1-열
np.random.seed(1)
...
# 예 : 나이 데이터를 가진 리스트이 경우
ages = [0, 2, 10, 21, 23, 37, 31, 61, 20, 41, 32, 101]
# cut() 을 사용해서 카테고리(열) 값으로 변경할 수 있음
# bins 인수로 분류하는 기준값을 지정함, 기준을 넘는 값음 NaN 이 됨
